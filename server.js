...
// API: Get all founders/admins for trainer message composer
app.get('/api/trainer/founders', requireAuth, async (req, res) => {
  try {
    const currentStaff = await getCurrentStaff(req);
    if (!currentStaff) {
      return res.status(404).json({ success: false, error: 'Staff profile not found' });
    ...
    // Return only active founder/admin staff records
    const founders = await Staff.find({
...
app.post('/api/messages/send', requireAuth, parseJson, async (req, res) => {
  try {
...
app.post('/api/team-messages/send', requireAuth, parseJson, async (req, res) => {
  try {
    const currentStaff = await getCurrentStaff(req);
    if (!currentStaff) {
      return res.status(404).json({ success: false, error: 'Staff profile not found' });
